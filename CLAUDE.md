...
- **No router.** `src/App.jsx` owns an `activeTab` state and renders one of five pages from `src/pages/` (Dashboard, Expenses, Budget, Splurge, Settings) behind `BottomNav`. App-level flows (welcome-income modal, month-end rollover, streak/celebration toasts) are also orchestrated in `App.jsx`.
- **Data layer is hooks over Supabase.** Each hook in `src/hooks/` wraps a set of tables and exposes data + mutation functions that pages receive as props (no global store). `useFinanceData` is the aggregate for a given month: categories, income, income_sources, expenses (joined to categories). It seeds `DEFAULT_CATEGORIES` on a user's first load, and treats `income_sources` as optional so a missing table degrades gracefully. `useAuth` handles Supabase Google OAuth; `useUserStats` holds streaks/theme; `useSplurge` loads lazily only when its tab is active.
...
- **Pure logic lives in `src/lib/`** (safe-to-spend, weekly digest, splurge recommendations, streaks) — keep business rules there, not in components.
...
Schema lives in `supabase/schema.sql` with incremental changes in `migrations-v2.sql` / `migrations-v3.sql`; these are run manually in the Supabase SQL Editor (no migration tooling). Tables: `income`, `categories` (with `budget_limit`, `sort_order`), `expenses`, `income_sources`, `splurge_goals`, `splurge_contributions`, `user_stats`. All rows are user-scoped — new tables need RLS policies matching the existing ones.
